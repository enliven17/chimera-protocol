...
export const BetDialog: React.FC<BetDialogProps> = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (market) {
      ...
      if (betAmountNum < minBetNum) {
        toast.error(`Minimum bet amount is ${minBetNum} PYUSD`);
        return;
      }
      
      if (betAmountNum > maxBetNum) {
        toast.error(`Maximum bet amount is ${maxBetNum} PYUSD`);
        return;
      }
    }
  ...
  const getBetValidation = () => {
    ...
    if (betAmountNum < minBetNum) {
      return { 
        isValid: false, 
        error: `Minimum bet: ${minBetNum} PYUSD` 
      };
    }
    ...
    if (betAmountNum > maxBetNum) {
      return { 
        isValid: false, 
        error: `Maximum bet: ${maxBetNum} PYUSD` 
      };
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-xs text-gray-400">
                ...
                <span>{formatBalance(balance?.value)} PYUSD</span>
              </div>
              ...
              {market && (
                <div className="flex justify-between text-xs text-gray-400">
                  <span>Bet Range:</span>
                  <span>{market.minBet} - {market.maxBet} PYUSD</span>
                </div>
              )}
                ...
                <div className="flex items-center space-x-2 mb-2">
                  {isConfirming ? (
                    ...
                    <>
                      ...
                      <p className="text-yellow-400 text-sm font-medium">Transaction Confirmed!</p>
                    </>
                  ) : (
                    <>
      ...
      <Dialog open={showSuccess} onOpenChange={(open) => {
        ...
        <DialogContent className="bg-gradient-to-br from-[#1A1F2C] to-[#151923] border-gray-800/50 text-white max-w-md">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold flex items-center space-x-2 text-yellow-400">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-800/30 rounded-lg p-4 space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-white font-bold">{successData.amount} PYUSD</span>
                </div>
                
                <div className="flex justify-between items-center">
                  <span className="text-gray-400 text-sm">Shares Received:</span>
                  <span className="text-[#eab308] font-bold">{successData.shares}</span>
                </div>
              </div>
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex space-x-2">
                      ...
                      <Button
                        ...
                        className="h-6 px-2 text-xs bg-gray-700/50 border-gray-600 text-gray-300 hover:bg-gray-600/50"
                        onClick={() => {
                          window.open(`https://hashscan.io/testnet/transaction/${successData.txHash}`, '_blank');
                        }}
                      >
              ...
              {/* Next Steps */}
              <div className="bg-gradient-to-r from-[#eab308]/10 to-[#ca8a04]/10 rounded-lg p-4 border border-[#eab308]/20">
                <h3 className="font-semibold text-sm text-[#eab308] mb-2">What's Next?</h3>
